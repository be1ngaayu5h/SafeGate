...
import { QrCode, Calendar, Filter, RefreshCw, Home } from "lucide-react";
...
export default function ResidentQRVisitorHistoryPage() {
  ...
  // Hardcoded flat number for resident (A101 as per existing pattern)
  const residentFlatNo = "A101";
  ...
  const fetchQRVisitors = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:8080/qr-visitor/history/${residentFlatNo}`);
      if (response.ok) {
  ...
  return (
    <div className="container mx-auto p-6 max-w-6xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">My QR Visitor History</h1>
        <p className="text-gray-600">View all QR visitors you've created for your flat</p>
      </div>
      ...
      <Card className="mb-6">
        <CardContent className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-gray-600">Your Flat</p>
              <p className="font-semibold">{residentFlatNo}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <span>My QR Visitor Records</span>
        ...
        <CardContent>
          {loading ? (
            <div className="text-center py-8">
              ...
              <p>Loading your QR visitor history...</p>
            </div>
          ) : filteredVisitors.length === 0 ? (
            <div className="text-center py-8">
              <QrCode className="w-12 h-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500">No QR visitors found for your flat</p>
              <p className="text-sm text-gray-400 mt-2">
                Create your first QR visitor request to get started
              </p>
            </div>
          ) : (
